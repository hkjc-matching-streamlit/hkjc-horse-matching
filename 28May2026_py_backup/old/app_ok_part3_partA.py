...
def fetch_hkjc_racecard_chinese(race_date: str, racecourse: str, race_no: str):
    ...
    url = f"https://racing.hkjc.com/racing/information/Chinese/Racing/RaceCard.aspx?{params}"
    ...
    for raw in text_nodes:
        ...
        if "後備馬匹" in raw:
            in_reserve_section = True
            continue

        cleaned = normalize_horse_name(raw)

        if 1 < len(cleaned) <= 12 and re.search(r"[\u4e00-\u9fff]", cleaned):
            if cleaned not in ["馬名", "退出", "排位表", "賽事資料", "賽馬資訊", "後備馬匹", "第", "場"]:
                if in_reserve_section:
                    if cleaned not in seen_reserve:
                        seen_reserve.add(cleaned)
                        reserve_horses.append(cleaned)
                else:
                    if cleaned not in seen_main:
                        seen_main.add(cleaned)
