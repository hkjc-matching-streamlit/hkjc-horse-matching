...
def fetch_hkjc_racecard_chinese(race_date: str, racecourse: str, race_no: str):
    ...
    # 精準抓法：只抓 href 含 horseid 的 <a> tag 馬名
    ...
    for a in all_horse_links:
        ...
        for parent in a.parents:
            ...
            if "後備馬匹" in parent_text or "後 備 馬 匹" in parent_text:
                in_reserve = True
                break
...
def compare_local_with_racecard(local_df: pd.DataFrame, main_horses: list[str], reserve_horses: list[str]):
    ...
    active_df["命中正選"] = active_df["標準化馬名"].isin(main_set)
    active_df["命中後備"] = active_df["標準化馬名"].isin(reserve_set)

    matched_main = active_df[active_df["命中正選"]].copy()
    matched_reserve = active_df[active_df["命中後備"]].copy()
    unmatched = active_df[~active_df["命中正選"] & ~active_df["命中後備"]].copy()
...
def compare_local_with_racecard_day(local_df: pd.DataFrame, all_main: list[tuple], all_reserve: list[tuple]):
    ...
    active_df["命中正選場次"] = active_df["標準化馬名"].apply(
        lambda x: ", ".join(str(n) for n in main_map.get(x, []))
    )
    active_df["命中後備場次"] = active_df["標準化馬名"].apply(
        lambda x: ", ".join(str(n) for n in reserve_map.get(x, []))
    )

    active_df["命中正選"] = active_df["命中正選場次"] != ""
    active_df["命中後備"] = active_df["命中後備場次"] != ""

    matched_main = active_df[active_df["命中正選"]].copy()
    matched_reserve = active_df[~active_df["命中正選"] & active_df["命中後備"]].copy()
    unmatched = active_df[~active_df["命中正選"] & ~active_df["命中後備"]].copy()
...
if racecard_btn:
    if df_current.empty:
        ...
        try:
            ...
            r2.metric("命中正選", len(matched_main))
            r3.metric("命中後備", len(matched_reserve))
            r4.metric("未命中", len(unmatched_rc))
...
if day_scan_btn:
    if df_current.empty:
        ...
        try:
            ...
            d2.metric("命中正選", len(matched_main_day))
            d3.metric("命中後備", len(matched_reserve_day))
            d4.metric("未命中", len(unmatched_day))

            st.markdown("### ✅ 命中正選（顯示出賽場次）")
            if not matched_main_day.empty:
                st.dataframe(
                    matched_main_day[["馬匹名字", "電郵地址", "命中正選場次", "命中後備場次", "建立日期", "最後更新日期"]],
                    use_container_width=True,
                    hide_index=True
                )
            else:
                st.info("整個賽日沒有命中正選馬匹。")

            st.markdown("### 🟡 命中後備（顯示後備場次）")
            if not matched_reserve_day.empty:
                st.dataframe(
                    matched_reserve_day[["馬匹名字", "電郵地址", "命中正選場次", "命中後備場次", "建立日期", "最後更新日期"]],
                    use_container_width=True,
                    hide_index=True
                )
            else:
                st.info("整個賽日沒有命中後備馬匹。")
